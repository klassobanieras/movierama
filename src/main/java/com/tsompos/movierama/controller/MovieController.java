...
public class MovieController {
    ...
    @GetMapping(MOVIES_URL + "/{userId}")
    public Page<MovieProjection> getAllMovies(@PathVariable String userId, @PageableDefault(page = 0, size = 20)
    @SortDefault.SortDefaults({@SortDefault(sort = "publishedDate", direction = Sort.Direction.DESC)}) Pageable pageable) {
        return movieRecommendationService.fetchAllMoviesOfUser(userId, pageable);
    }
    ...
    public MovieRecommendation createMovieRecommendation(@AuthenticationPrincipal Jwt principal,
        ...
        return movieRecommendationService.save(createMovie, principal.getClaimAsString("userId"));
    }
    ...
    public void setReaction(@AuthenticationPrincipal Jwt principal, @PathVariable Long movieId, @PathVariable String reaction) {
        movieRecommendationService.addReaction(movieId, principal.getClaimAsString("userId"),
    ...
    public void removeReaction(@AuthenticationPrincipal Jwt principal, @PathVariable Long movieId) {
        movieRecommendationService.removeReaction(movieId, principal.getClaimAsString("userId"));
    }
    ...
    public void switchReaction(@AuthenticationPrincipal Jwt principal, @PathVariable Long movieId) {
        movieRecommendationService.switchReaction(movieId, principal.getClaimAsString("userId"));
    }
